...
function generateAstGenerated(): string {
    ...
    // ── Header (with imports resolved from the schema model) ──
    // Types referenced by the generated declarations but not declared here are
    // hand-written in ast.ts; import them so the module type-checks.
    const astImports = ["Node", "NodeArray", ...unresolvedAstImports()];
    astImports.sort((a, b) => a.localeCompare(b));
    const header = `// Code generated by tools/scripts/tsc/generate-ts-ast.ts. DO NOT EDIT.
...
import type { ${astImports.join(", ")} } from "./ast.ts";`;

    return [header, body].join(eol);
}

// TypeScript names that are available without importing from ast.ts: built-in
// primitives, and enums/base types imported from other modules in the header.
const astBuiltinTypeNames = new Set([
    "any",
    "boolean",
    "never",
    "null",
...
// Walks a schema Type and records every named type it references in the
// generated TypeScript. Node and alias references contribute their name;
...
function collectTypeReferences(type: Type, into: Set<string>): void {
    switch (type.kind) {
        ...
        case "alias":
            ...
            break;
        case "union":
            for (const member of type.types) collectTypeReferences(member, into);
            break;
        case "list":
            collectTypeReferences(type.elementType, into);
            break;
        case "primitive": {
            const name = type.formatTypeScript();
            if (!astBuiltinTypeNames.has(name)) into.add(name);
            break;
        }
        case "kind":
        case "typeParameter":
            // Emitted as `SyntaxKind.*` or a locally-declared name; nothing to import.
            break;
    }
}

// The set of type names declared within ast.generated.ts itself. Mirrors the
// declarations emitted by generateAstGenerated so references to them are not
// mistaken for hand-written ast.ts types.
function declaredAstNames(): Set<string> {
    const declared = new Set<string>();
    for (const { name } of api.kindAliases()) declared.add(name);
    for (const base of api.bases()) {
        if (!goOnlyBases.has(base.key)) declared.add(baseTsName(base));
    }
    // Concrete node interfaces and variant-node union aliases both use node.name.
    for (const node of api.nodes()) {
        if (!node.handWritten) declared.add(node.name);
    }
    for (const alias of api.nodeAliases()) declared.add(alias.name);
    for (const variant of tsVariants) declared.add(variant.tsName);
    for (const node of api.nodes()) {
        for (const { name } of node.instantiationAliases) declared.add(name);
    }
    for (const listAlias of api.listAliases()) declared.add(listAlias.name);
    return declared;
}

// Type names referenced by ast.generated.ts but not declared within it, and so
// hand-written in ast.ts and needing an import. Derived from the schema model:
// the same Type objects generateAstGenerated emits are walked for references.
function unresolvedAstImports(): string[] {
    ...
    for (const base of api.bases()) {
        ...
        for (const field of base.fields) {
            if (field.noTS) continue;
            collectTypeReferences(field.type, referenced);
        }
    }
    for (const node of api.nodes()) {
        if (node.handWritten || isVariantNode(node)) continue;
        collectTypeReferences(node.kindType, referenced);
        for (const member of tsInterfaceMembers(node)) {
            collectTypeReferences(member.type, referenced);
        }
    }
    for (const alias of api.nodeAliases()) {
        if (alias.isUnion) {
    ...
    for (const listAlias of api.listAliases()) {
        if (listAlias.elementType) collectTypeReferences(listAlias.elementType, referenced);
    }

    const declared = declaredAstNames();
    return [...referenced]
        .filter(name => !declared.has(name) && !astBuiltinTypeNames.has(name))
...
function generateFactory(): string {
    ...
    // ── Header ──
    out.push(`// Code generated by tools/scripts/tsc/generate-ts-ast.ts. DO NOT EDIT.`);
    out.push("");
