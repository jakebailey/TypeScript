...
function tryOptimizeBin() {
    ...
    const binPath = path.join(binDir, process.platform === "win32" ? "tsc.exe" : "tsc");
...
function patchWindowsShims(packageDir, exePath) {
    for (const shimDir of getWindowsShimDirs(packageDir)) {
        patchWindowsShim(path.join(shimDir, "tsc"), exePath, writeShellShim);
        patchWindowsShim(path.join(shimDir, "tsc.cmd"), exePath, writeCmdShim);
        patchWindowsShim(path.join(shimDir, "tsc.ps1"), exePath, writePowerShellShim);
    }
}
