...
async function runWasi(wasmPath) {
    ...
    // Node 20-22 can instantiate the Go WASI binary, but crash during normal compiler I/O.
    if (nodeMajor < 23) {
...
function getWasiPaths(wasmPath) {
    ...
    const wasmRoot = path.parse(wasmPath).root;
    if (wasmRoot.toLowerCase() !== cwdRoot.toLowerCase()) {
        throw new Error("The WASI fallback requires the TypeScript package and current directory to be on the same Windows drive.");
    }

    const toGuestPath = hostPath => {
        const relative = path.relative(cwdRoot, hostPath).replaceAll("\\", "/");
