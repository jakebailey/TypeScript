    ...
    const getSourceFileWithCache: CompilerHost["getSourceFile"] | undefined = getSourceFile
        ? (fileName, languageVersionOrOptions, onError, shouldCreateNewSourceFile) => {
            ...
            const impliedNodeFormat: ResolutionMode = typeof languageVersionOrOptions === "object"
                ? languageVersionOrOptions.impliedNodeFormat : undefined;
...
function formatCodeSpan(
    ...
    for (let i = firstLine; i <= lastLine; i++) {
        ...
        if (hasMoreThanFiveLines && firstLine + 1 < i && i < lastLine - 1) {
            context += indent + formatColorAndReset(padLeft(ellipsis, gutterWidth), gutterStyleSequence)
                + gutterSeparator + host.getNewLine();
            i = lastLine - 1;
        }
        ...
        context += indent + formatColorAndReset(padLeft(i + 1 + "", gutterWidth), gutterStyleSequence)
            + gutterSeparator;
...
/** @internal */
export function getResolutionModeOverrideForClause(
    clause: AssertClause | undefined,
...
/** @internal */
export const typeReferenceResolutionNameAndModeGetter: ResolutionNameAndModeGetter<
...
export function getReferencedFileLocation(
    ...
    switch (kind) {
        case FileIncludeKind.Import:
            ...
            packageId = file.resolvedModules?.get(importLiteral.text, getModeForResolutionAtIndex(file, index))
        ...
        case FileIncludeKind.TypeReferenceDirective:
            ...
            packageId = file.resolvedTypeReferenceDirectiveNames?.get(
...
export function isProgramUptoDate(
    ...
    if (
        ...
        && forEachEntry(
            program.resolvedLibReferences,
            (_value, libFileName) => hasInvalidatedLibResolutions(libFileName),
        )
    ) return false;
...
export function createProgram(
    ...
    tracing?.push(tracing.Phase.Program, "createProgram", {
    ...
    if (structureIsReused !== StructureIsReused.Completely) {
                            ...
                            for (const fileName of parsedRef.commandLine.fileNames) {
                                if (!isDeclarationFileName(fileName) && !fileExtensionIs(fileName, Extension.Json)) {
                                    processProjectReferenceFile(
                                        ...
                                        { kind: FileIncludeKind.OutputFromProjectReference, index },
                                    );
                                }
                            }
                        }
        ...
        missingFilePaths = arrayFrom(
            mapDefinedIterator(filesByName.entries(), ([path, file]) => file === undefined ? path as Path : undefined),
        );
    ...
    function resolveTypeReferenceDirectiveNamesWorker<T extends FileReference | string>(
        ...
        const containingFileName = !isString(containingFile)
            ? getNormalizedAbsolutePath(containingFile.originalFileName, currentDirectory) : containingFile;
    ...
    function getRedirectReferenceForResolution(file: SourceFile) {
        ...
        if (
            !host.realpath || !options.preserveSymlinks || !stringContains(file.originalFileName, nodeModulesPathPart)
        ) return undefined;
    ...
    function resolveModuleNamesReusingOldState(
        ...
        if (oldSourceFile !== file && file.resolvedModules) {
            ...
            for (const moduleName of moduleNames) {
                const resolvedModule = file.resolvedModules.get(
                    moduleName.text,
        ...
        function moduleNameResolvesToAmbientModuleInNonModifiedFile(moduleName: StringLiteralLike): boolean {
            const resolutionToFile = getResolvedModule(
                oldSourceFile,
                moduleName.text,
                getModeForUsageLocation(file, moduleName),
            );
    ...
    function resolveTypeReferenceDirectiveNamesReusingOldState<T extends string | FileReference>(
        ...
        const oldSourceFile = !isString(containingFile)
            ? oldProgram && oldProgram.getSourceFile(containingFile.fileName) : undefined;
            ...
            if (oldSourceFile !== containingFile && containingFile.resolvedTypeReferenceDirectiveNames) {
                ...
                for (const typeDirectiveName of typeDirectiveNames as readonly FileReference[]) {
                    ...
                    const resolvedTypeReferenceDirective = containingFile.resolvedTypeReferenceDirectiveNames.get(
                        getTypeReferenceResolutionName(typeDirectiveName),
                        getModeForFileReference(typeDirectiveName, containingFile.impliedNodeFormat),
                    )!;
        ...
        for (let i = 0; i < typeDirectiveNames.length; i++) {
            ...
            if (canReuseResolutions) {
                ...
                const oldResolution = (!isString(containingFile) ? oldSourceFile?.resolvedTypeReferenceDirectiveNames
    ...
    function tryReuseStructureFromOldProgram(): StructureIsReused {
        ...
        for (const oldSourceFile of oldSourceFiles) {
            ...
            newSourceFile.packageJsonLocations = sourceFileOptions.packageJsonLocations?.length
                ? sourceFileOptions.packageJsonLocations : undefined;
        ...
        for (const { oldFile: oldSourceFile, newFile: newSourceFile } of modifiedSourceFiles) {
            ...
            const resolutionsChanged = hasChangesInResolutions(
                ...
                newSourceFile,
                resolutions,
                oldSourceFile.resolvedModules,
                moduleResolutionIsEqualTo,
                moduleResolutionNameAndModeGetter,
            );
            if (resolutionsChanged) {
                structureIsReused = StructureIsReused.SafeModules;
                newSourceFile.resolvedModules = zipToModeAwareCache(
                    newSourceFile,
                    moduleNames,
                    resolutions,
                    moduleResolutionNameAndModeGetter,
            ...
            if (typeReferenceResolutionsChanged) {
                structureIsReused = StructureIsReused.SafeModules;
                newSourceFile.resolvedTypeReferenceDirectiveNames = zipToModeAwareCache(
                    newSourceFile,
                    typesReferenceDirectives,
                    typeReferenceResolutions,
                    typeReferenceResolutionNameAndModeGetter,
    ...
    function emitWorker(
        ...
        const emitResolver = getTypeChecker().getEmitResolver(
            outFile(options) ? undefined : sourceFile,
            cancellationToken,
        );
    ...
    function getBindAndCheckDiagnosticsForFileNoCache(
        ...
        return runWithCancellationToken(() => {
            ...
            let checkDiagnostics = includeBindAndCheckDiagnostics
                ? typeChecker.getDiagnostics(sourceFile, cancellationToken) : emptyArray;
            if (isPlainJs) {
                ...
                switch (node.kind) {
                    ...
                    case SyntaxKind.ModuleDeclaration:
                        const moduleKeyword = node.flags & NodeFlags.Namespace
                            ? tokenToString(SyntaxKind.NamespaceKeyword) : tokenToString(SyntaxKind.ModuleKeyword);
    ...
    function processTypeReferenceDirectives(file: SourceFile) {
        ...
        for (let index = 0; index < typeDirectives.length; index++) {
            ...
            if (
                mode && getEmitModuleResolutionKind(options) !== ModuleResolutionKind.Node16
    ...
    function processImportedModules(file: SourceFile) {
        ...
        if (file.imports.length || file.moduleAugmentations.length) {
            ...
            const optionsForFile =
                (useSourceOfProjectReferenceRedirect ? getRedirectReferenceForResolution(file)?.commandLine.options
                    : undefined) || options;
    ...
    function verifyCompilerOptions() {
        ...
        if (options.preserveValueImports && getEmitModuleKind(options) < ModuleKind.ES2015) {
            createDiagnosticForOptionName(
                Diagnostics.Option_0_can_only_be_used_when_module_is_set_to_es2015_or_later,
                ...
                const referenceInfo = forEachProjectReference(
                    ...
                    (resolvedRef, parent, index) =>
                        resolvedRef === referencedResolvedRef
                            ? { sourceFile: parent?.sourceFile || options.configFile!, index } : undefined,
                );
    ...
    for (let i = 0; i < projectReferences.length; i++) {
        ...
        if (ref.prepend && resolvedRefOpts && resolvedRefOpts.options) {
            ...
            const { jsFilePath, sourceMapFilePath, declarationFilePath, declarationMapPath, buildInfoPath } =
                getOutputPathsForBundle(resolvedRefOpts.options, /*forceDtsPaths*/ true);
