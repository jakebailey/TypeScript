...
export function createPrinter(printerOptions: PrinterOptions = {}, handlers: PrintHandlers = {}): Printer {
    ...
    var nodeToGeneratedName: Map<Node, string>; // Map of generated names for specific nodes.
    var nodeToGeneratedPrivateName: Map<Node, string>; // Map of generated names for specific nodes.
    ...
    function reset() {
        nodeToGeneratedName = new Map();
        nodeToGeneratedPrivateName = new Map();
    ...
    function generateNameCached(node: Node, privateName: boolean, flags?: GeneratedIdentifierFlags, prefix?: string | GeneratedNamePart, suffix?: string) {
        const cache = privateName ? nodeToGeneratedPrivateName : nodeToGeneratedName;
        let name = cache.get(node);
        if (name === undefined) {
            name = generateNameForNode(node, privateName, flags ?? GeneratedIdentifierFlags.None, formatGeneratedNamePart(prefix, generateName), formatGeneratedNamePart(suffix));
            cache.set(node, name);
        }
        return name;
    }
