...
export const parseBaseNodeFactory: BaseNodeFactory = {
    ...
    createBasePrivateIdentifierNode: kind =>
        new (PrivateIdentifierConstructor
            || (PrivateIdentifierConstructor = objectAllocator.getPrivateIdentifierConstructor()))(kind, -1, -1),
...
namespace Parser {
    ...
    function reparseTopLevelAwait(sourceFile: SourceFile) {
        ...
        while (start !== -1) {
            ...
            const diagnosticEnd = diagnosticStart >= 0 ? findIndex(savedParseDiagnostics, diagnostic =>
                diagnostic.start >= nextStatement.pos, diagnosticStart) : -1;
    ...
    function parseInitializer(): Expression | undefined {
        return parseOptional(SyntaxKind.EqualsToken)
            ? parseAssignmentExpressionOrHigher(/*allowReturnTypeInArrowFunction*/ true) : undefined;
    }
    ...
    function parsePropertyDeclaration(
        ...
        const exclamationToken = !questionToken && !scanner.hasPrecedingLineBreak()
            ? parseOptionalToken(SyntaxKind.ExclamationToken) : undefined;
        const type = parseTypeAnnotation();
        ...
        function parseJSDocCommentWorker(start = 0, length: number | undefined): JSDoc | undefined {
            ...
            function skipWhitespaceOrAsterisk(): string {
                ...
                while (
                    ...
                    || token() === SyntaxKind.WhitespaceTrivia || token() === SyntaxKind.NewLineTrivia
                ) {
                ...
                switch (node.kind) {
                    ...
                    default:
                        return isTypeReferenceNode(node) && isIdentifierNode(node.typeName)
                            && node.typeName.escapedText === "Object" && !node.typeArguments;
                }
            }
            ...
            function parseTypeTag(
                ...
                const comments = indent !== undefined && indentText !== undefined
                    ? parseTrailingTagComments(start, getNodePos(), indent, indentText) : undefined;
            ...
            function parseSatisfiesTag(
                ...
                const comments = margin !== undefined && indentText !== undefined
                    ? parseTrailingTagComments(start, getNodePos(), margin, indentText) : undefined;
            ...
            function parseChildParameterOrPropertyTag(
                ...
                indent: number,
                name?: EntityName,
            ): JSDocTypeTag | JSDocPropertyTag | JSDocParameterTag | JSDocTemplateTag | false {
...
export function isDeclarationFileName(fileName: string): boolean {
    return fileExtensionIsOneOf(fileName, supportedDeclarationExtensions)
        || (fileExtensionIs(fileName, Extension.Ts) && stringContains(getBaseFileName(fileName), ".d."));
}
