    ...
    public getOutputPath(path: string, ext: string): string {
        if (this.options.outFile || this.options.out) {
            ...
            const outDir = ext === ".d.ts" || ext === ".d.mts" || ext === ".d.cts"
                    || (ext.endsWith(".ts") || ts.stringContains(ext, ".d."))
                ? this.options.declarationDir || this.options.outDir : this.options.outDir;
            if (outDir) {
...
export function compileFiles(
    ...
    if (
        compilerOptions.target === undefined && compilerOptions.module !== ts.ModuleKind.Node16
