        ...
        cache?.forEach((resolved, key, mode) =>
            baselines.push(
                `${key}: ${mode ? ts.getNameOfCompilerOptionValue(mode, ts.moduleOptionDeclaration.type) + ": " : ""}${
                    JSON.stringify(resolved, /*replacer*/ undefined, 2)
                }`,
            )
        );
    }
    ...
    it("successful if change affects a single module of a package", () => {
        const files = [
            ...
            { name: "/node_modules/b/internal.d.ts", text: SourceText.New("", "", "export const b = 1;") },
            {
                ...
                text: SourceText.New("", "", JSON.stringify({ name: "b", version: "1.2.3" })),
            },
        ];
...
/// <reference types="typerefs1" />`;
    ...
    it("fails if config path changes", () => {
        ...
        const baselines: string[] = [];
        baselineProgram(baselines, program1);
        const program2 = updateProgram(program1, ["a.ts"], {
            target,
            module: ts.ModuleKind.CommonJS,
            ...
            const files: NamedSourceText[] = [
                ...
                },
                {
                    ...
                    text: SourceText.New(
                        "",
                        "",
                        JSON.stringify({ name: "x", version: options ? options.bVersion : "1.2.3" }),
                    ),
                },
                {
                    name: root,
