    ...
    baselineParseConfig({
        ...
        input: () => [{
            ...
            baselineParsed: (baseline, parsed) => {
                ...
                if (parsed.wildcardDirectories) ts.getOwnKeys(parsed.wildcardDirectories).forEach(dir => wildcardDirectories![dir] = `WatchDirectoryFlags.${ts.WatchDirectoryFlags[parsed.wildcardDirectories![dir]]}`);
                baseline.push(
                    "Result",
                    jsonToReadableText({
                        ...parsed,
