    ...
    function getRuleProvider(placeOpenBraceOnNewLineForFunctions: boolean): ts.formatting.FormatContext {
        return ts.formatting.getFormatContext(
            placeOpenBraceOnNewLineForFunctions
                ? { ...ts.testFormatSettings, placeOpenBraceOnNewLineForFunctions: true } : ts.testFormatSettings,
            notImplementedHost,
        );
    }
        ...
        runSingleFileTest(
            ...
            (sourceFile, changeTracker) => {
                changeTracker.replaceNode(
                    ...
                    findVariableStatementContaining("y", sourceFile),
                    createTestClass(),
                    { trailingTriviaOption: ts.textChanges.TrailingTriviaOption.Exclude, suffix: newLineCharacter },
                );
            },
        );
        runSingleFileTest(
