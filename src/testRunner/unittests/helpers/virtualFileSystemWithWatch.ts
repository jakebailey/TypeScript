...
class Callbacks {
    ...
    log() {
        ...
        return `${this.callbackType} callback:: count: ${details.length}`
            + (details.length ? "\r\n" + details.join("\r\n") : "");
    }
...
export class TestServerHost implements server.ServerHost, FormatDiagnosticsHost, ModuleResolutionHost {
    ...
    serializeWatches(baseline: string[] = []) {
        ...
        this.serializedWatchedFiles = serializeMultiMap(
            baseline,
            "PolledWatches",
            this.watchedFiles,
            this.serializedWatchedFiles,
        );
        this.serializedFsWatches = serializeMultiMap(baseline, "FsWatches", this.fsWatches, this.serializedFsWatches);
        this.serializedFsWatchesRecursive = serializeMultiMap(
            baseline,
            "FsWatchesRecursive",
            this.fsWatchesRecursive,
            this.serializedFsWatchesRecursive,
        );
...
function serializeMultiMap<T>(
    baseline: string[],
    caption: string,
    multiMap: MultiMap<string, T>,
    serialized: Map<string, T[]> | undefined,
) {
