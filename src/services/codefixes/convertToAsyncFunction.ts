...
function convertToAsyncFunction(
    ...
    const returnStatements = functionToConvertRenamed.body && isBlock(functionToConvertRenamed.body)
        ? getReturnStatementsWithPromiseHandlers(functionToConvertRenamed.body, checker) : emptyArray;
...
function getExplicitPromisedTypeOfPromiseReturningCallExpression(
    node: PromiseReturningCallExpression<"then" | "catch" | "finally">,
    callback: Expression,
    checker: TypeChecker,
) {
...
*/
function renameCollidingVarNames(
    nodeToRename: FunctionLikeDeclaration,
    checker: TypeChecker,
    synthNamesMap: Map<string, SynthIdentifier>,
): FunctionLikeDeclaration {
...
function finishCatchOrFinallyTransform(
    node: PromiseReturningCallExpression<"then" | "catch" | "finally">,
    transformer: Transformer,
    tryStatement: TryStatement,
    possibleNameForVarDecl: SynthIdentifier | undefined,
    continuationArgName?: SynthBindingName,
) {
...
function transformFinally(
    ...
    const inlinedCallback = transformCallbackArgument(
        onFinally,
        hasContinuation,
        /*continuationArgName*/ undefined,
        /*inputArgName*/ undefined,
        node,
        transformer,
    );
    if (hasFailed()) return silentFail(); // shortcut out of more work
...
function transformCatch(
    ...
    const catchClause = factory.createCatchClause(
        inputArgName && getSynthesizedDeepClone(declareSynthBindingName(inputArgName)),
        factory.createBlock(inlinedCallback),
    );
 ...
 * @param continuationArgName The argument name for the continuation that follows this call.
 */
function transformThen(
    node: PromiseReturningCallExpression<"then">,
    onFulfilled: Expression | undefined,
    onRejected: Expression | undefined,
    transformer: Transformer,
    hasContinuation: boolean,
    continuationArgName?: SynthBindingName,
    ...
    switch (func.kind) {
        ...
        case SyntaxKind.Identifier: // identifier includes undefined
            ...
            const synthCall = factory.createCallExpression(
                getSynthesizedDeepClone(func as Identifier | PropertyAccessExpression),
            ...
            const varDeclOrAssignment = createVariableOrAssignmentOrExpressionStatement(
                continuationArgName,
                factory.createAwaitExpression(synthCall),
                getExplicitPromisedTypeOfPromiseReturningCallExpression(parent, func, transformer.checker),
            );
        ...
        case SyntaxKind.ArrowFunction: {
            ...
            if (isBlock(funcBody)) {
                ...
                const inlinedStatements = isFixablePromiseHandler(funcBody, transformer.checker)
                    ? transformReturnStatementWithFixablePromiseHandler(
                        transformer,
                        factory.createReturnStatement(funcBody),
                        hasContinuation,
                        continuationArgName,
                    )
                    : emptyArray;
...
function removeReturns(
    ...
    prevArgName: SynthBindingName | undefined,
    transformer: Transformer,
    seenReturnStatement: boolean,
): readonly Statement[] {
    const ret: Statement[] = [];
        ...
        if (isReturnStatement(stmt)) {
            if (stmt.expression) {
                const possiblyAwaitedExpression = isPromiseTypedExpression(stmt.expression, transformer.checker)
                    ? factory.createAwaitExpression(stmt.expression) : stmt.expression;
...
function createSynthBindingPattern(
    ...
    elements: readonly SynthBindingName[] = emptyArray,
    types: Type[] = [],
): SynthBindingPattern {
