...
function mapSuggestionDiagnostic(errorCode: number) {
    switch (errorCode) {
        ...
        case Diagnostics
            .Property_0_implicitly_has_type_any_but_a_better_type_for_its_get_accessor_may_be_inferred_from_usage.code:
            return Diagnostics
        ...
        case Diagnostics
            .Property_0_implicitly_has_type_any_but_a_better_type_for_its_set_accessor_may_be_inferred_from_usage.code:
            return Diagnostics
...
function doChange(
    ...
    if (
        !isParameterPropertyModifier(token.kind) && token.kind !== SyntaxKind.Identifier
        && token.kind !== SyntaxKind.DotDotDotToken && token.kind !== SyntaxKind.ThisKeyword
    ) {
        return undefined;
    }
    ...
    switch (errorCode) {
        ...
        case Diagnostics.Variable_0_implicitly_has_type_1_in_some_locations_where_its_type_cannot_be_determined.code:
            if (
                (isVariableDeclaration(parent) && markSeen(parent)) || isPropertyDeclaration(parent)
        ...
        case Diagnostics.Variable_0_implicitly_has_an_1_type.code: {
            ...
            if (
                ...
                && markSeen(symbol.valueDeclaration)
            ) {
    ...
    if (isInJSFile(containingFunction)) {
        ...
        if (needParens) {
            changes.insertNodeAfter(
                sourceFile,
                last(containingFunction.parameters),
...
function annotate(
    ...
    if (typeNode) {
        if (isInJSFile(sourceFile) && declaration.kind !== SyntaxKind.PropertySignature) {
            const parent = isVariableDeclaration(declaration) ? tryCast(declaration.parent.parent, isVariableStatement)
                : declaration;
            if (!parent) {
                return;
            }
...
function inferTypeFromReferences(
    ...
    function combineTypes(inferences: readonly Type[]): Type {
        ...
        const priorities: Priority[] = [
            ...
            {
                high: t =>
                    !(t.flags & (TypeFlags.Nullable | TypeFlags.Any | TypeFlags.Void))
    ...
    function inferFromSignatures(genericSig: Signature, usageSig: Signature, typeParameter: Type) {
        ...
        for (let i = 0; i < genericSig.parameters.length; i++) {
            ...
            let genericParamType = genericParam.valueDeclaration
                ? checker.getTypeOfSymbolAtLocation(genericParam, genericParam.valueDeclaration) : checker.getAnyType();
