...
func TestWatchLogicalDirectoryDescendants(t *testing.T) {
	...
	wm := NewWatchManager(io.Discard, filesystem.DirectoryExists, filesystem)
	assert.NilError(t, wm.ReconcileWatches([]string{"/repo/linked", "/repo/file.ts"}, map[string]bool{"/physical": true}, nil, "/repo/linked"))
	for _, kind := range []fswatch.EventKind{fswatch.EventUpdate, fswatch.EventDelete} {
		wm.onWatchEvents([]fswatch.Event{{Path: "/physical/nested/new.ts", Kind: kind}}, nil)
		changes := wm.DrainEvents()
		assert.DeepEqual(t, changes.Changes, map[string]fswatch.EventKind{
