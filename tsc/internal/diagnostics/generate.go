...
func main() {
	...
	locEmbedOutput := flag.String("loc-embed", "", "path to the output loc_embed_generated.go file")
	locNoembedOutput := flag.String("loc-noembed", "", "path to the output loc_noembed_generated.go file")
	locDir := flag.String("locdir", "", "directory to write locale .json.gz files")
	flag.Parse()

	if *diagnosticsOutput == "" || *locOutput == "" || *locEmbedOutput == "" || *locNoembedOutput == "" || *locDir == "" {
		flag.Usage()
		return
	}
	...
	// Generate localizations files (shared, embed, noembed)
	locBuf, locEmbedBuf, locNoembedBuf := generateLocalizations(knownKeys, *locDir)
	...
	for _, output := range []struct {
		...
		{*locOutput, locBuf},
		{*locEmbedOutput, locEmbedBuf},
		{*locNoembedOutput, locNoembedBuf},
	} {
		formatted, err = format.Source(output.buf.Bytes())
		if err != nil {
			log.Fatalf("failed to format localizations output %s: %v", output.path, err)
			return
		}

		if err := os.WriteFile(output.path, formatted, 0o666); err != nil {
			log.Fatalf("failed to write localizations output %s: %v", output.path, err)
			return
		}
	}
}
...
func generateLocalizations(knownKeys map[string]bool, locDir string) (shared *bytes.Buffer, embed *bytes.Buffer, noembed *bytes.Buffer) {
	shared = new(bytes.Buffer)
	embed = new(bytes.Buffer)
	noembed = new(bytes.Buffer)

	// --- Shared file (no build tag) ---
	shared.WriteString("// Code generated by generate.go; DO NOT EDIT.\n")
	shared.WriteString("\n")
	shared.WriteString("package diagnostics\n")
	shared.WriteString("\n")
	shared.WriteString("import (\n")
	shared.WriteString("\t\"compress/gzip\"\n")
	shared.WriteString("\t\"strings\"\n")
	shared.WriteString("\t\"golang.org/x/text/language\"\n")
	shared.WriteString("\t\"github.com/microsoft/TypeScript/tsc/internal/json\"\n")
	shared.WriteString(")\n")

	// --- Embed file ---
	embed.WriteString("//go:build !noembed\n\n")
	embed.WriteString("// Code generated by generate.go; DO NOT EDIT.\n")
	embed.WriteString("\n")
	embed.WriteString("package diagnostics\n")
	embed.WriteString("\n")
	embed.WriteString("import (\n")
	embed.WriteString("\t_ \"embed\"\n")
	embed.WriteString("\t\"sync\"\n")
	embed.WriteString(")\n")

	// --- Noembed file ---
	noembed.WriteString("//go:build noembed\n\n")
	noembed.WriteString("// Code generated by generate.go; DO NOT EDIT.\n")
	noembed.WriteString("\n")
	noembed.WriteString("package diagnostics\n")
	noembed.WriteString("\n")
	noembed.WriteString("import (\n")
	noembed.WriteString("\t\"path/filepath\"\n")
	noembed.WriteString("\t\"runtime\"\n")
	noembed.WriteString("\t\"sync\"\n")
	noembed.WriteString("\t\"testing\"\n")
	noembed.WriteString("\n")
	noembed.WriteString("\t\"github.com/microsoft/TypeScript/tsc/internal/bundled\"\n")
	noembed.WriteString("\t\"github.com/microsoft/TypeScript/tsc/internal/tspath\"\n")
	noembed.WriteString("\t\"github.com/microsoft/TypeScript/tsc/internal/vfs/osvfs\"\n")
	noembed.WriteString(")\n")
	...
	shared.WriteString("\tnil, // English (default)\n")
	for _, loc := range locales {
		fmt.Fprintf(shared, "\t%s,\n", loc.varName)
	}
	shared.WriteString("}\n")

	// Generate helper function for decompressing locale data
	shared.WriteString("\nfunc loadLocaleData(data string) map[Key]string {\n")
	shared.WriteString("\tgr, err := gzip.NewReader(strings.NewReader(data))\n")
	shared.WriteString("\tif err != nil {\n")
	shared.WriteString("\t\tpanic(\"failed to create gzip reader: \" + err.Error())\n")
	shared.WriteString("\t}\n")
	shared.WriteString("\tdefer gr.Close()\n")
	shared.WriteString("\tvar result map[Key]string\n")
	shared.WriteString("\tif err := json.UnmarshalRead(gr, &result); err != nil {\n")
	shared.WriteString("\t\tpanic(\"failed to unmarshal locale data: \" + err.Error())\n")
	shared.WriteString("\t}\n")
	shared.WriteString("\treturn result\n")
	shared.WriteString("}\n")

	// --- Embed: go:embed directives and OnceValue loaders ---

	for _, loc := range locales {
		fmt.Fprintf(embed, "\n//go:embed loc/%s\n", loc.filename)
		fmt.Fprintf(embed, "var %sData string\n", loc.varName)
		fmt.Fprintf(embed, "\nvar %s = sync.OnceValue(func() map[Key]string {\n", loc.varName)
		fmt.Fprintf(embed, "\treturn loadLocaleData(%sData)\n", loc.varName)
		embed.WriteString("})\n")
	}

	// --- Noembed: load locale files from disk ---

	// locPath resolves the directory containing locale .json.gz files.
	// During tests, it uses the source loc directory; otherwise, it uses the
	// directory next to the executable (populated during build).
	noembed.WriteString("\nvar locPath = sync.OnceValue(func() string {\n")
	noembed.WriteString("\tif testing.Testing() {\n")
	noembed.WriteString("\t\t_, filename, _, ok := runtime.Caller(0)\n")
	noembed.WriteString("\t\tif !ok {\n")
	noembed.WriteString("\t\t\tpanic(\"diagnostics: could not get current filename\")\n")
	noembed.WriteString("\t\t}\n")
	noembed.WriteString("\t\treturn tspath.NormalizeSlashes(filepath.Join(filepath.Dir(filepath.FromSlash(filename)), \"loc\"))\n")
	noembed.WriteString("\t}\n")
	noembed.WriteString("\treturn bundled.ExecutableDir()\n")
	noembed.WriteString("})\n")

	noembed.WriteString("\nfunc loadLocaleFile(filename string) map[Key]string {\n")
	noembed.WriteString("\tcontents, ok := osvfs.FS().ReadFile(tspath.CombinePaths(locPath(), filename))\n")
	noembed.WriteString("\tif !ok {\n")
	noembed.WriteString("\t\treturn nil\n")
	noembed.WriteString("\t}\n")
	noembed.WriteString("\treturn loadLocaleData(contents)\n")
	noembed.WriteString("}\n")

	for _, loc := range locales {
		fmt.Fprintf(noembed, "\nvar %s = sync.OnceValue(func() map[Key]string {\n", loc.varName)
		fmt.Fprintf(noembed, "\treturn loadLocaleFile(%q)\n", loc.filename)
		noembed.WriteString("})\n")
	}

	return shared, embed, noembed
}
