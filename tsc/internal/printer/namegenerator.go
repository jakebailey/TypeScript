...
type NameGenerator struct {
	...
	nodeIdToGeneratedName              map[*ast.Node]string      // Map of generated names for specific nodes
	nodeIdToGeneratedPrivateName       map[*ast.Node]string      // Map of generated private names for specific nodes
...
func (g *NameGenerator) generateNameForNodeCached(node *ast.Node, privateName bool, flags GeneratedIdentifierFlags, prefix string, suffix string) string {
	...
	if *cache == nil {
		*cache = make(map[*ast.Node]string)
	}

	if name, ok := (*cache)[node]; ok {
		return name
	}
