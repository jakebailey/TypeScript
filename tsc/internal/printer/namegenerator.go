...
type NameGenerator struct {
	...
	nameGenerationScopeStack           core.Stack[nameGenerationScope]
	privateNameGenerationScopeStack    core.Stack[nameGenerationScope]
...
func (g *NameGenerator) PushScope(reuseTempVariableScope bool) {
	g.privateNameGenerationScopeStack.PushZero()
	if !reuseTempVariableScope {
		g.nameGenerationScopeStack.PushZero()
	}
}

func (g *NameGenerator) PopScope(reuseTempVariableScope bool) {
	if g.privateNameGenerationScopeStack.Len() > 0 {
		g.privateNameGenerationScopeStack.Pop()
	}
	if !reuseTempVariableScope && g.nameGenerationScopeStack.Len() > 0 {
		g.nameGenerationScopeStack.Pop()
	}
}

func (g *NameGenerator) getScope(privateName bool) *core.Stack[nameGenerationScope] {
	return core.IfElse(privateName, &g.privateNameGenerationScopeStack, &g.nameGenerationScopeStack)
}

func (g *NameGenerator) getScopeRef(privateName bool) *nameGenerationScope {
	stack := g.getScope(privateName)
	if stack.Len() == 0 {
		return nil
	}
	return stack.PeekRef()
}

func (g *NameGenerator) getOrPushScope(privateName bool) *nameGenerationScope {
	stack := g.getScope(privateName)
	if stack.Len() == 0 {
		stack.PushZero()
	}
...
func (g *NameGenerator) getTempFlags(privateName bool) tempFlags {
	scope := g.getScopeRef(privateName)
	if scope != nil {
...
func (g *NameGenerator) setTempFlags(privateName bool, flags tempFlags) {
	scope := g.getOrPushScope(privateName)
...
func (g *NameGenerator) isReservedName(name string, privateName bool) bool {
	...
	for i := stack.Len() - 1; i >= 0; i-- {
		if stack.At(i).reservedNames.Has(name) {
			return true
		}
	}
	return false
