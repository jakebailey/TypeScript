...
func TestWatchAliasDirectoryObservations(t *testing.T) {
	t.Parallel()
	for _, wildcard := range []bool{false, true} {
		t.Run(fmt.Sprintf("wildcard=%v", wildcard), func(t *testing.T) {
			t.Parallel()
			const mainURI = "file:///project/main.ts"
			main := `import {value} from "./generated/value"; export {value};`
			config := `{"compilerOptions":{"noLib":true,"types":[]},"files":["main.ts"]}`
			if wildcard {
				main = "export {};"
				config = `{"compilerOptions":{"noLib":true,"types":[]},"files":["main.ts"],"include":["generated/**/*.ts"]}`
			}
			disk := vfstest.FromMap(map[string]string{
				"/project/tsconfig.json": config,
			...
			defer session.Close()
			session.DidOpenFile(ctx, mainURI, 1, main, lsproto.LanguageKindTypeScript)
			session.WaitForBackgroundTasks()
			initial := session.Snapshot()
			assert.Assert(t, initial.watchAliases != nil)
			assert.Assert(t, initial.watchAliases.Match(map[string]fswatch.EventKind{
				"/project/generated": fswatch.EventUpdate,
			}).NamespaceChanged, "directory observations must not be classified as known file updates")

			const source = "/project/generated/value.ts"
			assert.NilError(t, disk.WriteFile(source, "export const value = 1;"))
			session.DidChangeWatchedFiles(ctx, []*lsproto.FileEvent{{
				Uri: "file:///project/generated", Type: lsproto.FileChangeTypeCreated,
			}})
			service, err := session.GetLanguageService(ctx, mainURI)
			assert.NilError(t, err)
			file := service.GetProgram().GetSourceFile(source)
			assert.Assert(t, file != nil, "directory creation must discover new sources")
			assert.Equal(t, file.Text(), "export const value = 1;")
			session.WaitForBackgroundTasks()

			assert.NilError(t, disk.Remove(source))
			assert.NilError(t, disk.Remove("/project/generated"))
			session.DidChangeWatchedFiles(ctx, []*lsproto.FileEvent{{
				Uri: "file:///project/generated", Type: lsproto.FileChangeTypeDeleted,
			}})
			service, err = session.GetLanguageService(ctx, mainURI)
			assert.NilError(t, err)
			assert.Assert(t, service.GetProgram().GetSourceFile(source) == nil, "directory deletion must invalidate cached descendants")
		})
	}
}
