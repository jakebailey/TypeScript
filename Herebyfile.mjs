...
function buildTsgo(opts) {
    opts ||= {};
    const out = opts.out ?? path.resolve("./built/local/tsc");
    const env = { ...goBuildEnv, ...opts.env };
    return $({ cancelSignal: opts.abortSignal, env, cwd: "./tsc" })`go build ${goBuildFlags} ${opts.extraFlags ?? []} ${goBuildTags("noembed")} -o ${out} ./cmd/tsgo`;
}
