...
const reviewers = process.env.REQUESTING_USER ? [process.env.REQUESTING_USER]
    : ["weswigham", "sandersn", "RyanCavanaugh"];
const masterBranchname = `user-baseline-updates`;
const targetBranch = process.env.TARGET_BRANCH || "main";
const branchName = process.env.TARGET_FORK?.toLowerCase() === "microsoft"
        && (targetBranch === "main" || targetBranch === "refs/heads/main")
    ? masterBranchname
...
runSequence([
    ...
    ["git", [
        "commit",
        "-m",
        `"Update user baselines${+(process.env.SOURCE_ISSUE ?? 0) === 33716 ? " +cc @sandersn" : ""}"`,
    ]], // Commit all changes (ping nathan if we would post to CI thread)
...
gh.pulls.create({
    ...
    title: `🤖 User test baselines have changed`
        + (process.env.TARGET_BRANCH ? ` for ${process.env.TARGET_BRANCH}` : ""),
    head: `${userName}:${branchName}`,
    base: branchName === masterBranchname ? "main" : masterBranchname,
    body: `${
        process.env.SOURCE_ISSUE
            ? `This test run was triggerd by a request on https://github.com/Microsoft/TypeScript/pull/${process.env.SOURCE_ISSUE} `
                + "\n" : ""
...
cc ${reviewers.map(r => "@" + r).join(" ")}`,
    ...
    if (!process.env.SOURCE_ISSUE) {
        ...
        await gh.issues.createComment({
            ...
            repo: "TypeScript",
            body:
                `The user suite test run you requested has finished and _failed_. I've opened a [PR with the baseline diff from master](${r.data.html_url}).`,
        });
    }
}).then(() => {
